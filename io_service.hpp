...
#   define TEST_IORING_OP(opcode) do { for (int i = 0; i < probe->ops_len; ++i) { if (probe->ops[i].op == opcode && probe->ops[i].flags & IO_URING_OP_SUPPORTED) { puts("\t" #opcode); break; } } } while (0)
