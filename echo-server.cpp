...
void serve(io_coroutine& coro, int clientfd) {
...
#if USE_POLL
#   if USE_LINK
        coro.poll(clientfd, POLLIN, IOSQE_IO_LINK).detach();
#   else
        coro.poll(clientfd, POLLIN).await();
#   endif
#endif
#if USE_SPLICE
#   if USE_LINK
...
void accept_connection(io_coroutine& coro, uint16_t server_port) {
    ...
    if (listen(serverfd, MAX_CONN_SIZE * 2)) panic("listen", errno);
    fmt::print("Listening: {}\n", server_port);
...
int main(int argc, char *argv[]) {
    ...
    new io_coroutine(
        host,
        std::bind(accept_connection, std::placeholders::_1, server_port)
    );
    host.run();
}
